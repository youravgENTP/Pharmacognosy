...
import { collectionDocumentPatchSchema, uuidSchema, validCollectionDocument } from "@/lib/validators";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) { const authError = await authorizeApi("editor"); if (authError) return authError;
  ...
  const [collection] = await db.select({ kind: collections.kind }).from(collections).where(eq(collections.id, id)).limit(1);
  if (!collection) return NextResponse.json({ error: "Not found" }, { status: 404 });
  if (!validCollectionDocument(collection.kind, parsed.data.document)) return NextResponse.json({ error: "Spreadsheet Collections must contain exactly one table block." }, { status: 400 });
