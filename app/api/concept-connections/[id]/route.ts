...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (parsed.data.action?.startsWith("accept") && side) { const anchorId = side === "a" ? edge.anchorAId : edge.anchorBId; const [anchor] = await db.select().from(conceptAnchors).where(eq(conceptAnchors.id, anchorId)).limit(1); if (anchor) { if (side === "a") { patch.anchorASnapshot = endpointSnapshot(anchor); patch.historicalA = false; } else { patch.anchorBSnapshot = endpointSnapshot(anchor); patch.historicalB = false; } await db.update(conceptAnchors).set({ status: "healthy", updatedAt: new Date() }).where(eq(conceptAnchors.id, anchor.id)); } }
  if (parsed.data.action?.startsWith("historical") && side) { const anchorId = side === "a" ? edge.anchorAId : edge.anchorBId; if (side === "a") patch.historicalA = true; else patch.historicalB = true; await db.update(conceptAnchors).set({ status: "historical", updatedAt: new Date() }).where(eq(conceptAnchors.id, anchorId)); }
  if (parsed.data.action?.startsWith("reassign") && side && parsed.data.replacementAnchorId) { const [anchor] = await db.select().from(conceptAnchors).where(eq(conceptAnchors.id, parsed.data.replacementAnchorId)).limit(1); if (!anchor) return NextResponse.json({ error: "Replacement anchor not found" }, { status: 404 }); if (side === "a") { patch.anchorAId = anchor.id; patch.anchorASnapshot = endpointSnapshot(anchor); patch.historicalA = false; } else { patch.anchorBId = anchor.id; patch.anchorBSnapshot = endpointSnapshot(anchor); patch.historicalB = false; } }
