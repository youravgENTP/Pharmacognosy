...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = z.object({ name: z.string().trim().min(1).max(120).optional(), kind: z.enum(["pathway", "class", "subclass", "compound"]).optional(), description: z.string().max(1000).nullable().optional() }).safeParse(await request.json());
