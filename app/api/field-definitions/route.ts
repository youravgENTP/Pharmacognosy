...
export async function POST(request: Request) {
  ...
  if (existing) {
    if (!existing.active) {
      const [restored] = await db.update(fieldDefinitions).set({ active: true, updatedAt: new Date() }).where(eq(fieldDefinitions.id, existing.id)).returning();
  ...
  const [created] = await db.insert(fieldDefinitions).values({ name: parsed.data.name, kind: "custom", position: (last?.position ?? 99) + 1 }).returning();
