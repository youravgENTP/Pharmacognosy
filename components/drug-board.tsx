...
const importanceOrder: Importance[] = ["중요", "중간", "비중요", "연관"];
...
export function DrugBoard({ categories }: { categories: Category[] }) {
  ...
  return <>
    <header className="db-header">
      <div><p className="eyebrow">생약 데이터베이스 · {total} drugs</p><h1>Pharmacognosy</h1></div>
      <div className="db-tools"><div className="search-wrap"><Search size={18}/><input className="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="생약명, Latin name, 기원 검색" aria-label="생약 검색"/></div><div className="filter-anchor"><button className={`filter-button ${importanceFilters.size < 4 ? "active" : ""}`} onClick={() => setFilterOpen((value) => !value)}><SlidersHorizontal size={17}/> 필터{importanceFilters.size < 4 ? ` · ${importanceFilters.size}` : ""}</button>{filterOpen ? <div className="filter-popover"><strong>중요도</strong>{importanceOrder.map((value) => <button key={value} onClick={() => toggleFilter(value)}><span className={`filter-check ${importanceFilters.has(value) ? "checked" : ""}`}>{importanceFilters.has(value) ? <Check size={13}/> : null}</span>{value}</button>)}</div> : null}</div></div>
    </header>
...
function importanceClass(value: Importance) { return { 중요: "important", 중간: "medium", 비중요: "low", 연관: "related" }[value]; }
