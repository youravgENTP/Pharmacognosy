...
export type CurrentUser = { id: string; name: string; email: string; image?: string | null; role: UserRole };
...
export async function getCurrentUser(requestHeaders?: Headers): Promise<CurrentUser | null> {
  ...
  return { id: session.user.id, name: session.user.name, email: session.user.email, image: session.user.image, role: profile.role };
}
