...
import { categories, crudeDrugRelationships, crudeDrugs, families, relationshipTypes } from "@/lib/db/schema";
...
export async function getDrugProfile(id: string) {
  ...
  return { ...drug, sections: normalized, family: [drug.familyKorean, drug.familyScientific].filter(Boolean).join(" · ") || null, relatedDrugs, similarDrugs, availableDrugs };
}
